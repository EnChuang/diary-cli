...
def build_score_user_message(event: dict[str, Any]) -> str:
    ...
    return (
        ...
        + "\n\n## 指示\n"
        "請輸出評分 JSON（只評非使用者）。\n"
        "核心：分數＝使用者對此人的主觀觀感（職場溫度計，不是仇人榜／正義榜）。\n"
        "幅度：預設中帶約 −15～＋15；無「當面踩我／甩鍋給我／公開下不來台」時禁止動輒 −40 或 +40。\n"
        "搶功／小人得意 → 宜約 −10～−20；哭／賣慘／被害禁止加分，可微負但勿為反可憐而重扣。\n"
        "護我＋關心 → 正但克制（約 +10～+25）；同苦無互動 → 約 0。\n"
        "解氣的懲治者可微正；誠懇道歉且氣消 → 可中或微正，禁止強正洗白。\n"
        "「算了」常＝壓抑微負。情緒詞很重≠必須強負。\n"
        "無感／沒互動／只是背景 → 優先 0 附近；禁止僅因很吵／話多就自動微負。\n"
        "必讀整段；髒話／垃圾爛人／搞死／!!! 等會加重負向（同情節可差約 5～10 分）。\n"
        "禁止事件類型固定分；每一則依當下語氣獨立評。\n"
        "若相對父篇有反轉，反映觀感力道變化（仍守中帶預設）。"
    )
